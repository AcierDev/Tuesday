    ...
    <>
      ...
      <Card className="w-full max-w-3xl mx-auto">
        <CardHeader>
          ...
          <CardDescription>Select and upload PDF shipping labels to the server.</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            <div className="flex items-center justify-center w-full">
              <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-64 border-2 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                ...
                <Input id="dropzone-file" type="file" className="hidden" onChange={handleFileChange} accept=".pdf" multiple />
              </label>
            </div>
                ...
                <ul className="space-y-2">
                  {selectedFiles.map((file, index) => (
                    <li key={index} className="flex items-center justify-between p-2 bg-gray-100 rounded">
                      ...
                      <Button variant="ghost" size="icon" onClick={() => removeFile(index)} aria-label={`Remove ${file.name}`}>
                        <X className="w-4 h-4" />
                      </Button>
                    </li>
                  ))}
      ...
      <Card className="w-full max-w-3xl mx-auto mt-8">
        <CardHeader>
          ...
          <CardDescription>View and download previously uploaded PDF files.</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
