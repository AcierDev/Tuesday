...
export default function InventoryManagement() {
  ...
  const { inventory, isLoading, addItem, updateItem, deleteItem, getLowStockItems, getFilteredInventory } = useInventoryContext()
  ...
  if (isLoading) {
    return <div className="p-8 bg-background text-foreground dark:bg-gray-900">Loading inventory...</div>
  }
    ...
    <div className="p-8 bg-background text-foreground dark:bg-gray-900">
      ...
      {categoriesWithItems.map(category => (
        <div key={category} className="mt-8">
          <h2 className="text-2xl font-semibold mb-4 sticky top-0 bg-background dark:bg-gray-900 z-10 pt-3 pb-2">{category}</h2>
          <InventoryTable
