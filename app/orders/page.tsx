...
export default function OrderManagementPage() {
  ...
  const dueCounts = useMemo(() => {
    ...
    board.items_page.items.forEach((item) => {
      if (isItemDue(item)) {
        ...
        const design = item.values.find((v) =>
          v.columnName === "Design"
        )?.text || "";
        ...
        if (design.startsWith("Striped") && !isMini) counts.striped++;
          ...
          !design.startsWith("Striped") && !isMini &&
  ...
  const getUniqueGroupValues = useCallback((items: Item[], field: string) => {
    ...
    items.forEach((item) => {
      const value = item.values.find((v) => v.columnName === field)?.text ||
        "Other";
  ...
  const filteredGroups = useMemo(() => {
    ...
    board.items_page.items.filter((item) => !item.deleted && item.visible)
      .forEach((item) => {
        if (
          item.values.some((value) =>
            String(value.text || "").toLowerCase().includes(
          ...
          if (
            ...
            (settings.groupingField !== "Status" ||
              settings.showCompletedOrders || item.status !== ItemStatus.Done)
          ) {
            const design = item.values.find((v) =>
              v.columnName === "Design"
            )?.text || "";
            const size = item.values.find((v) =>
              v.columnName === "Size"
            )?.text || "";
            ...
            const shouldInclude = (() => {
              switch (currentMode) {
                ...
                case "geometric":
                  return !design.startsWith("Striped") &&
                    !design.startsWith("Tiled") && !isMini;
                case "mini":
                  return isMini;
                case "custom":
                  return !Object.values(ItemDesigns).includes(
    ...
    <div className="flex flex-col min-h-screen bg-white dark:bg-gray-900 text-black dark:text-white">
      ...
      <div className="relative">
        ...
        <Button
          ...
          aria-label={isWeeklyPlannerOpen
            ? "Close weekly planner"
            : "Open weekly planner"}
        >
          {isWeeklyPlannerOpen
