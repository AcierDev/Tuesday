...
export async function GET(request: Request) {
  ...
  if (!trackingNumber) {
    return NextResponse.json({ error: "Invalid tracking number" }, {
      status: 400,
    });
  }
  ...
  try {
    ...
    const tokenResponse = await axios.post(
      ...
      }).toString(),
      {
        headers: {
          ...
          "Authorization": "Basic " +
            Buffer.from(`${UPS_USERNAME}:${UPS_PASSWORD}`).toString("base64"),
        },
      },
    );
    ...
    if (!shipment) {
      console.error(
        "Shipment data not found in UPS response:",
    ...
    if (!packageInfo) {
      console.error(
        "Package information not found in shipment data:",
        shipment,
      );
    ...
    const referenceNumbers = packageInfo.referenceNumber?.map((ref: any) =>
      ref.number
    ) || [];
