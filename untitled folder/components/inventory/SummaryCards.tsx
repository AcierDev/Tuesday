...
export default function SummaryCards({ inventory, lowStockItems, countFilter, handleCountFilterChange }: SummaryCardsProps) {
  return (
      ...
      {Object.values(CountFrequency).map((frequency) => (
        ...
        >
          <Card className="w-full h-full border-none shadow-none bg-transparent">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">{frequency} Count Items</CardTitle>
              <BarChart className={`h-4 w-4`} />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{inventory?.filter(item => item.countFrequency === frequency).length}</div>
            </CardContent>
          </Card>
        </Button>
      ))}
