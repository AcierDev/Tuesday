  ...
  return (
    <div className="w-full max-w-4xl mx-auto bg-card dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
      <div className="flex items-center justify-between p-4 bg-none">
        <h2 className="text-2xl font-semibold text-foreground dark:text-gray-200">{format(month, 'MMMM yyyy')}</h2>
        <div className="flex space-x-2">
          <Button variant="outline" size="icon" onClick={() => changeMonth('prev')} className="text-foreground dark:text-gray-200 dark:hover:bg-gray-700">
            <ChevronLeft className="h-5 w-5" />
          </Button>
          <Button variant="outline" size="icon" onClick={() => changeMonth('next')} className="text-foreground dark:text-gray-200 dark:hover:bg-gray-700">
      ...
      <div className="p-4">
        <div className="grid grid-cols-7 gap-2 mb-2">
          {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
            <div key={day} className="text-center font-medium text-sm text-muted-foreground dark:text-gray-400">
              {day}
            </div>
          ))}
        </div>
            ...
            return (
              <Button
                ...
                className={cn(
                  ...
                  !isSameMonth(day, month) && "text-muted-foreground dark:text-gray-500 opacity-50",
                  isToday(day) && "border-primary dark:border-blue-400",
                  isSelected && "bg-primary dark:bg-blue-600 text-primary-foreground dark:text-white",
              ...
              >
                <span className={cn(
                  ...
                  isToday(day) && "bg-primary dark:bg-blue-400 text-primary-foreground dark:text-gray-900 rounded-full w-6 h-6 flex items-center justify-center"
                )}>
                  {format(day, 'd')}
                </span>
                {itemCount > 0 && (
