...
export function DetailsTab(
  { isMobile, filteredRequirements }: DetailsTabProps,
) {
  return (
      ...
      <div className={cn("space-y-6", isMobile ? "p-4" : "p-6")}>
            ...
            >
              {filteredRequirements.map(([color, requirement]) => (
                ...
                >
                  ...
                  >
                    Hardware: {requirement.hardwareBag}
                  </p>
                  <p
                    className={cn(
        ...
        <Card className="bg-background dark:bg-gray-800">
          <CardContent className={cn(isMobile ? "p-4" : "p-6")}>
            ...
            >
              {Object.entries(BOX_COLORS).map((
                [size, { color, count, hardwareBag, mountingRail }],
              ) => (
                <div
                  key={size}
                  className={cn(
                ...
                >
                  <span className="font-semibold block text-foreground dark:text-gray-200">
                    {size}
                  </span>
