      ...
      <div className={cn("space-y-6", isMobile ? "p-4" : "p-6")}>
        <div className={cn("grid gap-6", isMobile ? "grid-cols-2" : "grid-cols-1 md:grid-cols-2 lg:grid-cols-3")}>
          <Card className="bg-background dark:bg-gray-800">
            <CardContent className={cn("flex flex-col justify-center", isMobile ? "p-4" : "p-6")}>
              <h3 className={cn("font-semibold mb-2 text-foreground dark:text-gray-200", isMobile ? "text-sm" : "text-lg")}>Total Boxes</h3>
              <p className={cn("font-bold text-foreground dark:text-white", isMobile ? "text-2xl" : "text-4xl")}>{totalBoxes}</p>
            </CardContent>
          </Card>
          <Card className="bg-background dark:bg-gray-800">
            <CardContent className={cn("flex flex-col justify-center", isMobile ? "p-4" : "p-6")}>
              <h3 className={cn("font-semibold mb-2 text-foreground dark:text-gray-200", isMobile ? "text-sm" : "text-lg")}>Box Colors</h3>
        ...
        <Card className="bg-background dark:bg-gray-800">
          <CardContent className={cn(isMobile ? "p-4" : "p-6")}>
            <h3 className={cn("font-semibold mb-4 text-foreground dark:text-gray-200", isMobile ? "text-sm" : "text-lg")}>Box Color Overview</h3>
            <div className={cn(
              "grid gap-4",
              isMobile ? "grid-cols-2" : "grid-cols-2 md:grid-cols-3 lg:grid-cols-4"
            )}>
              {filteredRequirements.map(([color, requirement]) => (
                <div key={color} className="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg">
                  <div 
                    className={cn(
