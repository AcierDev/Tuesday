...
import { UserPlus, UserX, XCircle } from "lucide-react";
...
export function DropdownCell(
  { item, columnValue, onUpdate, board }: DropdownCellProps,
) {
  ...
  useEffect(() => {
    if (columnValue.credit && columnValue.credit.length > 0) {
      setSelectedCredits(
        columnValue.credit.map((credit) => INITIALS_MAP[credit]).filter((
          initial,
        ): initial is CreditOption => !!initial),
      );
    } else {
      setSelectedCredits([]);
    }
  ...
  const handleCancelCredits = useCallback(() => {
    const initials = columnValue.credit?.map((credit) =>
      INITIALS_MAP[credit]
    ).filter((initial): initial is CreditOption =>
      !!initial
    ) || [];
