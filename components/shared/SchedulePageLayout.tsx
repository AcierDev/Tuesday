      ...
      >
        ...
        <div className="flex-grow overflow-auto">
          {tabs.map((tab) => (
            <TabsContent
              key={tab.value}
              className="h-full mt-0 p-4 flex-grow"
              value={tab.value}
            >
              {tab.content}
            </TabsContent>
      ...
      <div className="flex flex-col space-y-6 flex-grow">
        {viewMode === "week" ? (
          ...
          <>
            ...
            >
              <div className="flex-grow lg:w-2/3">{renderCalendarView()}</div>
              <div
                ref={tabsCardRef}
                className="flex-shrink-0 lg:w-5/12 overflow-hidden flex flex-col"
              >
                {renderTabs()}
              </div>
            </div>
