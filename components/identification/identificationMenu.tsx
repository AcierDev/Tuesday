...
export function UserIdentificationMenu() {
  ...
  useEffect(() => {
    if (isVisible) {
      ...
      if (currentUser) {
        const index = employees.findIndex(e => EMPLOYEE_MAP[e] === currentUser)
        if (index !== -1) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (selectedUser && passwordProtectedUsers.includes(EMPLOYEE_MAP[selectedUser])) {
        if (e.key === 'Enter') {
          e.preventDefault()
          handleLogin()
  ...
  }, [isVisible, employees, focusedIndex, selectedUser, passwordProtectedUsers])
                  ...
                  <AnimatePresence>
                    {employees.map((credit, index) => (
                      <EmployeeAvatar
                        ...
                        isPasswordProtected={passwordProtectedUsers.includes(EMPLOYEE_MAP[credit])}
                      />
                    ))}
                  </AnimatePresence>
                  <AnimatePresence>
                    {!isLogoutHovered && (
                      <motion.div
                        ...
                        className={`w-32 h-32 ${theme === 'dark' ? 'bg-gray-700' : 'bg-white'} rounded-full absolute inset-0 m-auto flex flex-col items-center justify-center shadow-lg`}
                      >
                        <Users className={`w-12 h-12 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`} />
        ...
        {currentUser ? (
          <>
            <Avatar className="w-12 h-12">
              <AvatarImage src={OPTION_IMAGES[INITIALS_MAP[currentUser]]} alt={currentUser} />
              <AvatarFallback>{currentUser.split(' ').map(n => n[0]).join('')}</AvatarFallback>
            </Avatar>
