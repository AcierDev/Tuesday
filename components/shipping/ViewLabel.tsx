...
export function ViewLabel({ orderId }: { orderId: string }) {
  ...
  const updateItemTracker = async (orderId: string, tracker: Tracker) => {
    if (!board) return;
    const initialItem = board?.items_page.items.find(
      (item) => item.id === orderId
    );
    ...
    const handleManualTrackingSubmit = async () => {
      ...
      try {
        ...
        await updateItemTracker(orderId, tracker);
            ...
            <TabsContent value="view">
              {pdfExists && pdfUrls.length > 0 ? (
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    No shipping labels have been uploaded for this order yet.
                    You can upload them in the "Manage Labels" tab.
                  </AlertDescription>
                </Alert>
              )}
            </TabsContent>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setManualCarrier(
                        e.target.value as TrackingInfo["carrier"]
                      )
                    }
