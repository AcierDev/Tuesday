...
export default function InventoryHistoryChart(
  { item }: InventoryHistoryChartProps,
) {
  ...
  const averageQuantity =
    chartData.reduce((sum, data) => sum + data.quantity, 0) / chartData.length;
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button
          variant="outline"
          size="sm"
          className="border-none bg-primary text-black hover:bg-primary/90 bg-gray-300 dark:bg-gray-900 dark:text-white"
        >
          View History
        </Button>
      </DialogTrigger>
