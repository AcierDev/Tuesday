      ...
      <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row items-center justify-between py-4 space-y-4 sm:space-y-0">
          ...
          <div className="flex justify-center w-full sm:w-2/4">
            ...
            >
              {["all", "geometric", "striped", "tiled", "mini", "custom"].map((
                mode,
              ) => (
                <ToggleGroupItem
                  key={mode}
          ...
          <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4 w-full sm:w-1/4 sm:justify-end">
            <div
              className={`relative w-full sm:w-64 transition-all duration-300 ease-in-out ${
                isSearchActive ? "scale-105 shadow-lg" : ""
              }`}
            >
              <Search
                className={`absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 dark:text-gray-500 transition-all duration-300 ${
                  isSearchActive ? "scale-110" : ""
                }`}
              />
              <Input
                ref={searchInputRef}
                className={`pl-10 pr-4 py-2 w-full border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600 dark:focus:ring-blue-400 transition-all duration-300 ${
                  isSearchActive ? "bg-blue-50 dark:bg-gray-600" : ""
                }`}
