...
export const NewItemModal = (
  { isOpen, onClose, onSubmit }: NewItemModalProps,
) => {
  ...
  const [optionalFields, setOptionalFields] = useState<
    Record<ColumnTitles, string>
  >(() => {
    const fields: Record<ColumnTitles, string> = {} as Record<
      ColumnTitles,
      string
    >;
  ...
  const [customInputs, setCustomInputs] = useState<
    Record<ColumnTitles, boolean>
  >({} as Record<ColumnTitles, boolean>);
              ...
              {!customInputs[ColumnTitles.Size]
                ? (
                  <div className="col-span-3 flex items-center space-x-2">
                    <Select
                      value={size}
                      onValueChange={(value) => setSize(value as ItemSizes)}
                    >
                    ...
                    <Button
                      size="icon"
                      variant="outline"
                      onClick={() => toggleCustomInput(ColumnTitles.Size)}
                      className="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200"
                    >
              ...
              {!customInputs[ColumnTitles.Design]
                ? (
                  <div className="col-span-3 flex items-center space-x-2">
                    <Select
                      value={design}
                      onValueChange={(value) => setDesign(value as ItemDesigns)}
                    >
                      <SelectTrigger className="w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">
                    ...
                    <Button
                      size="icon"
                      variant="outline"
                      onClick={() => toggleCustomInput(ColumnTitles.Design)}
                      className="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200"
                    >
                      <Pencil className="h-4 w-4" />
                    </Button>
            ...
            ? (
              <div className="grid gap-4 py-4">
                {Object.values(ColumnTitles).filter((title) =>
                  !["Customer Name", "Size", "Design", "Due Date"].includes(
                    title,
                  )
                ).map((title) => {
                          ...
                          ? (
                            <div className="col-span-3 flex items-center space-x-2">
                              <Select
                                ...
                                onValueChange={(value) =>
                                  handleOptionalFieldChange(title, value)}
                              >
                                <SelectTrigger className="w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">
                                  <SelectValue
                  ...
                  return (
                    ...
                    >
                      ...
                      <Input
                        ...
                        value={optionalFields[title]}
                        onChange={(e) =>
                          handleOptionalFieldChange(title, e.target.value)}
                      />
                    </div>
                  );
                })}
