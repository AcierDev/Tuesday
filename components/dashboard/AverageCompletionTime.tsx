...
export function AverageCompletionTimeChart({ board, timeRange }: { board: Board; timeRange: TimeRange }) {
  ...
  const data = useMemo(() => {
    ...
    board.items_page.items.forEach((item: Item) => {
      if (item.status === ItemStatus.Done && !item.deleted && item.completedAt && item.createdAt) {
        const completionTime = (item.completedAt - item.createdAt) / (1000 * 60 * 60) // Convert to hours
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      <LineChart data={data}>
        ...
        />
        <Line type="monotone" dataKey="averageTime" stroke={colors.line} strokeWidth={2} dot={false} />
      </LineChart>
    </ResponsiveContainer>
  )
}