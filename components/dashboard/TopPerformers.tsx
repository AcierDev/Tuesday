      ...
      if (
        ...
        if (isInRange(date)) {
          ...
          gluedColumn.credit.forEach((employee: string) => {
            performerData[employee] = (performerData[employee] || 0) +
              squaresPerPerson;
          });
        }
      }
      ...
      <Table className="flex-grow">
        ...
        <TableBody>
          {data.map((performer) => (
            <TableRow
              key={performer.name}
              className={selectedEmployee === performer.name
          ...
          {data.length < 5 &&
            [...Array(5 - data.length)].map((_, index) => (
              <TableRow key={`empty-${index}`}>
                ...
                <TableCell className="text-right text-gray-400 dark:text-gray-600">
                  &mdash;
                </TableCell>
              </TableRow>
            ))}
