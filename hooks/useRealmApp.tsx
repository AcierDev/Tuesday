...
interface RealmContextType {
  ...
  boardCollection:
    | globalThis.Realm.Services.MongoDB.MongoDBCollection<Board>
    | null;
  cuttingHistoryCollection:
    | globalThis.Realm.Services.MongoDB.MongoDBCollection<CuttingData>
    | null;
  inventoryCollection:
    | globalThis.Realm.Services.MongoDB.MongoDBCollection<InventoryItem>
    | null;
  isLoading: boolean;
}
...
export function RealmAppProvider({ children }) {
  ...
  const [boardCollection, setBoardCollection] = useState<
    globalThis.Realm.Services.MongoDB.MongoDBCollection<Board> | null
  >(null);
  const [cuttingHistoryCollection, setCuttingHistoryCollection] = useState<
    globalThis.Realm.Services.MongoDB.MongoDBCollection<CuttingData> | null
  >(null);
  const [inventoryCollection, setIntentoryCollection] = useState<
    globalThis.Realm.Services.MongoDB.MongoDBCollection<InventoryItem> | null
  >(null);
  ...
  useEffect(() => {
    ...
    const initRealm = async () => {
      ...
      try {
        ...
        const userCollection = mongo.db("react-web-app").collection(
          process.env.NEXT_PUBLIC_MODE!,
        );
        const cuttingHistoryCollection = mongo.db("react-web-app").collection(
          "cuttingHistory-" + process.env.NEXT_PUBLIC_MODE,
        );
        const inventoryCollection = mongo.db("react-web-app").collection(
